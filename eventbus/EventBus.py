...
class EventBus(object):
    ...
    def create_eventbus(self):
        ...
        xsub 和xpub构建事件传输的总线
        """
        self.xsubsocket = EventBus.CONTEXT.socket(zmq.XSUB)
        self.xsubsocket.bind(EventBus.XSUB_ADDR_PORT)

        self.xpubsocket = EventBus.CONTEXT.socket(zmq.XPUB)
        self.xpubsocket.setsockopt(zmq.XPUB_VERBOSE, 1)
        self.xpubsocket.bind(EventBus.XPUB_ADDR_PORT)
